...
class OdooboReportByAuthor(models.TransientModel):
    _name = "odoobo.report.by.author"
    _description = "Kardex / Informe por Venta de Publicaciones por Autor"
    ...
    def action_export_xlsx(self):
        ...
        sheet = workbook.add_worksheet("Kardex por Autor")
        sheet.set_landscape()
        sheet.fit_to_pages(1, 0)
        sheet.freeze_panes(6, 0)

        fmt_title = workbook.add_format({"bold": True, "font_size": 14, "align": "center", "valign": "vcenter", "border": 1})
        fmt_header = workbook.add_format({"bold": True, "align": "center", "valign": "vcenter", "text_wrap": True, "border": 1, "bg_color": "#E7E9ED"})
        ...
        fmt_money = workbook.add_format({"border": 1, "num_format": '#,##0.00 "Bs."'})
        fmt_author = workbook.add_format({"bold": True, "border": 1, "bg_color": "#F2F2F2"})
        fmt_total = workbook.add_format({"bold": True, "border": 1, "bg_color": "#E7E9ED", "num_format": "0.00"})
        fmt_total_money = workbook.add_format({"bold": True, "border": 1, "bg_color": "#E7E9ED", "num_format": '#,##0.00 "Bs."'})

        widths = [7, 48, 14, 14, 18, 15, 14, 18, 18, 18]
        for col, width in enumerate(widths):
            sheet.set_column(col, col, width)

        sheet.merge_range(0, 0, 1, 9, "INFORME POR VENTA DE PUBLICACIONES / KARDEX POR AUTOR", fmt_title)
        ...
        headers = [
            "Nro", "TÍTULO", "CANTIDAD\nRECIBIDA", "CANTIDAD\nDEVUELTA", "DEPÓSITO\nLEGAL",
            "TOTAL VENTAS\nREALIZADAS", "PVP", "PRECIO ACUERDO\nCONTRATO", "TOTAL VENTAS Bs", "SALDO EN\nCONSIG. O ALMACÉN",
        ]
